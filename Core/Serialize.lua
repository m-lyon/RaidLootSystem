...
function Serialize.encodeOpen(roundId, tierCount, secondsLeft, items, lootMode)
    local rows = {}
    for i = 1, #items do
        local item = items[i]
    ...
    local fields = { roundId, tierCount, secondsLeft, body }
...
function Serialize.decodeOpen(body)
    ...
    return { roundId = roundId, tierCount = tierCount,
