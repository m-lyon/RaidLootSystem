# Spec 002 — Round protocol

**Modules:** `Modules/Round.lua` (host), `Modules/Client.lua` (client), `Modules/Comms.lua`
**Depends on:** 000, 001
...
The lifecycle of a **round**: opening it, collecting submissions, keeping every client's view in
sync, closing it, and aborting it. Who is allowed to do what, and what happens when the network
or the raid misbehaves.

**Out of scope:** which items go into a round (004), how a winner is computed (003), what the
windows look like (005, 006).

## 2. What a round is

One round covers **every rollable item from a single loot source**, resolved in one go. Six
epics off a boss is one round with six columns, not six sequential rolls. This is the single
...
- Clients **drop** any of those ops arriving from a non-ML sender, and log it. There is no
  scenario in which a second client should be driving a round.
- Only one round may be open at a time. A second `OPEN` while one is live is a bug on the host;
...
3. No round is currently open.

The host then freezes `tierCount` and `timerSeconds` from its settings, builds the item list,
persists the round locally, broadcasts `OPEN`, and announces to raid chat (spec 004 handles
the item list, `Announce` handles the message).

**Tier count is frozen at open.** A `CFG` change mid-round is refused by the host UI with an
explanation; changes only take effect on the next round.

## 5. Submitting

A client builds its entry set locally (005) and sends `SUBMIT` with the **complete** set for the
round — submissions are idempotent replacements, not deltas. That makes revision trivial and
...
|---|---|
| Round id matches the open round | Drop the whole message silently (stale client) |
| Round state is `OPEN` | Drop, reply nothing — the timer has expired |
| `itemIdx` exists in the round | Drop that entry |
...
- A player may re-submit freely while the round is `OPEN`. Each `SUBMIT` fully replaces their
  previous entries.
...
| Code | Trigger |
|---|---|
| `ML_CHANGED` | Master looter changed while a round was open |
...
**Aborted rounds are never migrated to a new host.** A new host would have to reconstruct
entries it never received, and state-recovery bugs in a loot addon cost people items at the
worst possible moment. The new master looter restarts the round from the item-link path (004).

Aborts are written to history with their reason, so a round never simply vanishes.

If the host disconnects without sending `ABORT`, clients time out an open round **60 seconds
after `endsAt`** with no `RESULT`, mark it aborted locally as `HOST_LEFT`, and log it.

## 10. Resync

A client that joins mid-round, reloads its UI, or detects a gap sends `SYNC`. The host replies
...
- Opening a round with 6 items produces exactly one `OPEN`, correctly chunked, and all clients
  render 6 columns.
...
- Changing master looter mid-round aborts with `ML_CHANGED` on every client, and the entries are
  written to history as aborted.
- A client that `/reload`s mid-round recovers full state via `SYNC` within 5 seconds.
