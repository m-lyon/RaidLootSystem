...
| `CFG` | host → all | `tierCount^timerSeconds^lootMode` | Settings changed between rounds. Applied to the **campaign record**, not a display field: the campaign owns how the group plays it, so the next master looter inherits it (012 §9) |
